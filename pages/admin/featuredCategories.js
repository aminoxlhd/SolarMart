...
function FeaturedCategories() {
  ...
  const [{ loading, error, categories, loadingCreate, successDelete, loadingDelete }, dispatch] = useReducer(reducer, {
    loading: true,
    categories: [],
    error: '',
  });
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const { data } = await axios.get(`/api/admin/featuredCategories`, {
  ...
  const deleteHandler = async (categoryId) => {
    ...
    try {
      ...
      await axios.delete(`/api/admin/featuredCategories/${categoryId}`, {
        headers: { authorization: `Bearer ${userInfo.token}` },
      });
                ...
                {loading ? (
                  ...
                  <TableContainer>
                    <Table>
                      ...
                      <TableBody>
                        {categories.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt)).map((category) => (
                          <TableRow key={category._id}>
                            <TableCell>{category._id.substring(20, 24)}</TableCell>
                            ...
                            <TableCell>{category.description.substring(0, 76)}...</TableCell>

                            <TableCell>
                              <NextLink
                                href={`/admin/featuredCategory/${category._id}`}
                                passHref
                              >
...
export default dynamic(() => Promise.resolve(FeaturedCategories), { ssr: false });
